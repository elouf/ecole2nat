...
class SessionListPage
{
    ...
    public function render(): void
    {
        ...
        <div class="wrap">
            <h1>
                <?php esc_html_e('Séances types', 'ecole2nat'); ?>
            </h1>

            <p>
                <a
                    href="<?php echo esc_url(
                        admin_url('admin.php?page=ecole2nat-session')
                    ); ?>"
            ...
            <div style="margin-top:20px;">
                ...
                <div class="postbox">
                    <div class="postbox-header">
                        <h2 class="hndle">
                            <?php esc_html_e('Séances existantes', 'ecole2nat'); ?>
                        </h2>
                    </div>
    ...
    {
        if (
            isset($_GET['e2n_action'], $_GET['session_id'])
            && sanitize_key(wp_unslash($_GET['e2n_action'])) === 'toggle_session'
        ) {
            $sessionId = absint($_GET['session_id']);
    ...
    private function renderTable(array $sessions): void
    {
        if ($sessions === []) {
            ...
            <p>
                <?php esc_html_e(
                    'Aucune séance type n’a encore été créée.',
                    'ecole2nat'
                ); ?>
            </p>
            <?php
                    ...
                    <tr>
                        <td>
                            ...
                            <?php if (!empty($session['objectives'])) : ?>
                                <p class="description">
                                    <?php echo esc_html($session['objectives']); ?>
                                </p>
                            <?php endif; ?>
                        </td>

                        <td>
                            <?php echo esc_html($session['category_name']); ?>
                        </td>

                        <td>
                            <?php
                            echo !empty($session['duration'])
                                ? esc_html((string) $session['duration']) . ' min'
                                : '—';
                            ?>
                        </td>
    ...
    private function renderNotice(): void
    {
        ...
        $messages = [
            'created' => [
                'success',
                __('La séance type a bien été créée.', 'ecole2nat'),
            ],
            'updated' => [
                'success',
                __('Le statut de la séance a bien été modifié.', 'ecole2nat'),
            ],
            'duplicate' => [
                'warning',
