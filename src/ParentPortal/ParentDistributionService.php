...
class ParentDistributionService
{
    ...
    {
        return $this->repository->distributionGroups();
    }
    ...
    public function sendForSwimmer(int $swimmerId): array
    {
        ...
        $body = $this->emailBody($swimmer, $portalUrl, $code);
        ...
        $this->repository->markDistributed(
            $swimmerId,
            'email',
            $email
        );
    ...
    public function sendSelected(array $swimmerIds): array
    {
        $results = [
            'sent' => 0,
            'failed' => 0,
    ...
    public function sendMissingByGroup(int $groupId): array
    {
        ...
        foreach ($this->swimmersByGroup($groupId) as $swimmer) {
            if (
                empty($swimmer['parent_access_distributed_at'])
