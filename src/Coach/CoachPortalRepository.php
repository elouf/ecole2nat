...
class CoachPortalRepository {
 ...
 public function sessionsForCategory(int $categoryId):array {global $wpdb;$t=Config::table('sessions');return $wpdb->get_results($wpdb->prepare("SELECT id,name,objectives FROM {$t} WHERE category_id=%d AND is_active=1 AND is_library=1 ORDER BY name",$categoryId),ARRAY_A)?:[];}
