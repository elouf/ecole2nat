...
class CoachPortal
{
    ...
    private function competitionBilling(int $competitionId): void
    {
        ...
        <?php if($rows===[]):?><p class="e2n-info"><?php esc_html_e('Aucun nageur engagé sur Extranat ne peut être facturé pour le moment.','ecole2nat'); ?></p><?php else:?><form method="post" data-e2n-billing-form><?php wp_nonce_field('e2n_competition_day_'.$competitionId); ?>
            ...
            <div class="e2n-billing-list"><?php foreach($rows as $row):$swimmerId=(int)$row['swimmer_id'];$mealQty=(int)($row['meal_quantity']??0);$nightQty=(int)($row['night_quantity']??0);$total=$mealQty*$mealPrice+$nightQty*$nightPrice;$locked=($row['invoice_status']??'')==='payment_declared';?><article class="e2n-billing-row" data-e2n-billing-row data-meal-price="<?php echo esc_attr((string)$mealPrice); ?>" data-night-price="<?php echo esc_attr((string)$nightPrice); ?>">
                <?php if($locked):?><input type="hidden" name="billing[<?php echo $swimmerId; ?>][meal_quantity]" value="<?php echo $mealQty; ?>"><input type="hidden" name="billing[<?php echo $swimmerId; ?>][night_quantity]" value="<?php echo $nightQty; ?>"><input type="hidden" name="billing[<?php echo $swimmerId; ?>][individual_comment]" value="<?php echo esc_attr((string)($row['individual_comment']??'')); ?>"><?php endif; ?>
    ...
    {
        $mealTotal=(int)$invoice['meal_quantity']*(float)$invoice['meal_unit_price'];$nightTotal=(int)$invoice['night_quantity']*(float)$invoice['night_unit_price']; ?>
